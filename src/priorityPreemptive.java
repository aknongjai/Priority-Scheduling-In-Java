...
class data{
    ...
    public void getProcesses(){
        System.out.println("Enter the Process Id,Process Arrival Time &Process Burst Time and priority");
        pId=s.nextInt();
        pAt=s.nextInt();
        pBt=s.nextInt();
        priority=s.nextInt();
...
public class priorityPreemptive {
    ...
    private static ArrayList<Integer> sac=new ArrayList<Integer>();//contains the priority;
    private static void collectorAndShorter(data ex[]){
        for(int i=0;i<=n;i++){
            sac.add(i,0);
        }
        for(int i=1;i<=n ;i++){
            if(ex[i].pAt<=tab && ex[i].theBt>0) {
                ...
                sac.set(i, ex[i].priority);//then add the priorities
            }
        }
    ...
    private static void completionTime(data ex[]){
        ...
        int count=0;
        for (; k <= n;) {
            data backup[]=new data[100];//backup before collector ;
            for(int q=1;q<=n;q++){
                backup[q]=ex[q];
            }
            collectorAndShorter(ex);
            for (int h = 1; h <= n; h++){
                if (sac.get(h) != 0) {
                    flag = false;
                    ff = true;//for preemptive,interruption
                    count = ex[h].pId;
